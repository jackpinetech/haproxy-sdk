...
class _Utils {
  ...
  static async commandAcrossAllProcesses(hapObjects, method, ...args) {
    return Promise.all(
      hapObjects.map(async (_haProxyProcess) => {
        return {
          [await _haProxyProcess.processNumber()]: await (args.length == 1
  ...
  static stat2dict(csvData) {
    ...
    heads = headers.slice(2).trim().split(",");
    ...
    csvData
      ...
      .forEach((line) => {
        ...
        if (parts[1] === "FRONTEND") {
          ...
          parts[1] === "BACKEND" &&
          !Object.prototype.hasOwnProperty.call(dicts["backends"], parts[0])
        ) {
