...
class _Backend {
   ...
   */
  async servers(name) {
    return this._hap_process.serversStats(this.name, this._iid)
      .then(_csvLines => {
        let return_list = [];
        if (name) {
          if (_csvLines.hasOwnProperty(name)) {
            return_list.push(new _Server(this, name, _csvLines[name].get('sid')));
          } else {
            return [];
          }
        } else {
          for (let _name in _csvLines) {
            return_list.push(new _Server(this, _name, _csvLines[_name].get('sid')));
          }
        }
        return return_list;
      });
  ...
  async statsData() {
    try {
      return this._hap_process.backendsStats(this._iid).then(stats => stats[this.name]['stats']);
    } catch (e) {
      try {
        return this._hap_process.backendsStats().then(stats => stats[this.name]['stats']);
      } catch (e) {}
    }
    return [];
  }
