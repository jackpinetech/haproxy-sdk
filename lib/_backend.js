...
class _Backend {
   ...
   */
  async servers(name) {
    let servers = await this._hap_process.serversStats(this.name, this._iid),
      return_list = [];

    if (name) {
      if (servers.hasOwnProperty(name)) {
        return_list.push(new _Server(this, name, servers[name].get('sid')));
      } else {
        return [];
      }
    } else {
      for (let _name in servers) {
        return_list.push(new _Server(this, _name, servers[_name].get('sid')));
      }
    }
